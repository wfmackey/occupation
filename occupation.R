...
lfs <- lfs %>% 
  mutate(lfs = factor(lfs, levels = lfs.order))
...
occ <- occ %>% 
  mutate(occ = factor(occ, levels = occ.order))
...
occ.combined <- occ.combined %>% 
                mutate(code =  if_else(is.na(married), gender, 
                               if_else(married == TRUE, paste0(gender,".mar"),
                                       paste0(gender,".unmar")))
                       # child = if_else(grepl("haschild", gender) == TRUE, TRUE, 
                       #         if_else(grepl("nochild", gender) == TRUE, FALSE,
                       #                 NA_integer_))
                       )
# 
# tester <- c("f.haschild", "f.nochild", "f", "m")
# 
#         if_else(grepl("haschild", tester) == TRUE, TRUE, FALSE)
#         
#         if_else(grepl("nochild", tester) == TRUE, FALSE,
#                 NA_integer_))
# 
# 
# testing <- occ.combined %>%  
#         mutate(child =  if_else(grepl("haschild", gender) == TRUE, TRUE,
#                         if_else(grepl("nochild", gender) == TRUE, FALSE,
#                 NA_integer_)))
# 
...
#5.2 LF participation (45-44, 55-64) bachelor (male, female) by married v unmarried
...
#6: Additional (non-Bommerange) charts ####
